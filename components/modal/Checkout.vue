  ...
  <div :class="[openModal ? 'fixed flex' : 'hidden', 'modal']">
    ...
    <div class="modal-wrapper">
      <div class="bg-grey_light flex items-center justify-between rounded-t-2xl p-5">
        ...
        <button class="delete" aria-label="close" @click="closeModal(false)"><IconClose /></button>
      </div>
      <section class="p-5 rounded-b-2xl">
        <div v-if="!isCheckoutSection">
          <div class="box" v-for="product in products" :key="product.id">
            <div>
              <p>{{ product.title }} {{ product.quantity > 0 ? ` - Qty: ${product.quantity}` : "" }}</p>
      ...
      <div class="m-4">
        <button
          ...
          class="rounded-xl p-3 bg-blue text-white w-full"
          @click="onNextBtn"
        >
          {{ buyLabel }}
        </button>
